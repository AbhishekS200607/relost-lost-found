...
function displayItems(items) {
    ...
    if (items.length === 0) {
        itemsGrid.innerHTML = '<div class="col-span-full text-center text-muted-foreground py-8"><p>No items found yet. Be the first to post a found item!</p></div>';
        return;
    }
...
function shareItem(itemId, category) {
    const url = `${window.location.origin}/index.html#item-${itemId}`;
